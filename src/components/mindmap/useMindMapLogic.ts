...
import { NODE_SPACING_X } from "./constants";
...
export const useMindMapLogic = (
  ...
  // 指定ノードとその全子孫を縦方向に移動
  const moveNodeAndDescendantsVertically = useCallback(
  ...
  const applyHierarchicalYAdjustment = useCallback(
    (parentId: string, upwardExpansion: number, downwardExpansion: number) => {
      // 全ての移動対象ノードとその移動量を収集
      ...
      if (nodesToMove.length > 0) {
        ...
        console.log(
          `Moving ${allNodesToMove.length} nodes (including descendants)`,
        );

        // 一括でY座標を更新
        setNodes((prev) =>
          prev.map((node) => {
            const moveInstruction = allNodesToMove.find(
              (m) => m.nodeId === node.id,
            );
            if (moveInstruction) {
              return {
  ...
  const calculateBalancedChildPositions = useCallback(
    (parentNode: Node, useFixedDistance = true): { x: number; y: number }[] => {
      const childCount = parentNode.children.length;
      if (childCount === 0) return [];

      const parentWidth =
        parentNode.width ||
        calculateNodeWidth(parentNode.content, !parentNode.parentId);
  ...
  // 子孫ノードを縦方向に移動
  const moveDescendantsVertically = useCallback(
    (parentNodeId: string, deltaY: number) => {
      if (deltaY === 0) return;
  ...
  // 親ノード変更後の子ノード位置調整
  const adjustChildPositionsAfterParentChange = useCallback(
    (parentId: string) => {
  ...
  // 子孫ノードを移動（X、Y両方向）
  const moveDescendants = useCallback(
    (parentNodeId: string, deltaX: number, deltaY: number) => {
