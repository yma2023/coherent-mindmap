      ...
      <div className="p-4 border-t border-white/30">
        ...
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-3">
          ...
          <p className="text-xs text-amber-700">
            {t("mindMap.unlimitedMapsCollaboration")}
          </p>
        </div>
      </div>
    </div>
        ...
        >
          ...
          {node.isEditing ? (
            <input
              ...
              autoFocus
              onChange={(e) =>
                onHandleEditingContentChange(node.id, e.target.value)
              }
            ...
            <span
              className={`px-1 py-1 rounded transition-colors block ${
                !node.parentId ? "text-2xl font-bold" : "text-lg font-medium"
              }`}
              style={{
                width: "auto",
...
export const Connections: React.FC<ConnectionsProps> = ({ connections }) => (
  <>
    {connections.map((connection) => {
      // 複合パス（直線 + ベジェ曲線）の場合
      if (
        connection.type === "composite" &&
        connection.lineEndX &&
        connection.lineEndY &&
        connection.curveStartX &&
        connection.curveStartY &&
        connection.controlX &&
        connection.controlY
      ) {
        return (
...
export const NavigationModeDisplay: React.FC<NavigationModeDisplayProps> = ({
  navigationMode,
}) => {
  const { t } = useTranslation();
  ...
  return (
    <div className="absolute top-20 right-4 bg-gradient-to-br from-blue-600 to-indigo-700 text-white rounded-xl shadow-xl border border-blue-500/20 backdrop-blur-sm z-50">
      ...
      <div className="px-4 py-3 space-y-3">
        ...
        <div className="space-y-2">
          <div className="text-sm font-semibold text-green-200 uppercase tracking-wider">
            {t("mindMap.keyboard.add")}
          </div>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="flex items-center space-x-2">
              <kbd className="px-2 py-1 bg-green-500/30 rounded text-xs font-mono">
                Tab
              </kbd>
              <span className="text-white/90">
                {t("mindMap.keyboard.addChildNode")}
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <kbd className="px-2 py-1 bg-green-500/30 rounded text-xs font-mono">
              ...
              <span className="text-white/90">
                {t("mindMap.keyboard.addSiblingNode")}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="text-sm font-semibold text-red-200 uppercase tracking-wider">
            {t("mindMap.keyboard.operations")}
          </div>
          <div className="grid grid-cols-2 gap-2 text-sm">
            <div className="flex items-center space-x-2">
              <kbd className="px-2 py-1 bg-red-500/30 rounded text-xs font-mono">
                Del
              </kbd>
              <span className="text-white/90">
                {t("mindMap.keyboard.deleteNode")}
              </span>
            </div>
            <div className="flex items-center space-x-2">
              <kbd className="px-2 py-1 bg-gray-500/30 rounded text-xs font-mono">
                Esc
              </kbd>
              <span className="text-white/90">
                {t("mindMap.keyboard.exitMode")}
              </span>
            </div>
          </div>
        </div>
