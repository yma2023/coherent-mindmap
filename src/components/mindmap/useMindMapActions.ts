  ...
  const handleSiblingDeletionWithDescendants = useCallback(
    (
      ...
      const getBlockExtent = (nodeId: string) => {
        ...
        if (!n)
          return { minY: 0, maxY: 0, centerY: 0, height: NODE_MIN_HEIGHT };
      ...
      // --- 削除対象ブロックの高さ（ノード＋子孫） ---
      const deletedBlockExtent =
        deletedNode && deletedNode.id
          ? getBlockExtent(deletedNode.id)
          : { minY: 0, maxY: 0, centerY: 0, height: NODE_MIN_HEIGHT };
      ...
      if (groupCount >= 4) {
        ...
        while (currentNode && currentNode.parentId) {
          ...
          const affectedIndex =
            grandParent.children?.indexOf(currentNode.id) ?? -1;
          if (affectedIndex === -1) break;

          for (
            let i = affectedIndex + 1;
            i < (grandParent.children?.length ?? 0);
            i++
          ) {
      ...
      // 条件分岐2: グループが3つ
        ...
        // ① 対象は既に除外された
        // 残り2つのノードを取得
        const remainingIds = remainingChildren.filter(
          (id) => id && id !== "__deleted_placeholder__",
        );
        if (remainingIds.length === 2) {
          ...
          const top =
            aCenter <= bCenter
              ? { id: idA, extent: extentA }
              : { id: idB, extent: extentB };
          const bottom =
            top.id === idA
              ? { id: idB, extent: extentB }
              : { id: idA, extent: extentA };
          ...
          const bottomNewCenter =
            parentNewY + bottom.extent.height / 2 + margin;
          ...
          if (bottomNode) {
            ...
            getDescendants(bottom.id).forEach((d) =>
              addMove(d.id, 0, bottomDy),
            );
          }
    ...
    (nodeId: string) => {
      ...
      setTimeout(() => {
        if (siblingHasDescendants) {
          ...
          // 削除による縮小量を計算（逆方向）
          const upwardReduction = Math.max(
            0,
            originalUpwardExtent - newUpwardExtent,
          );
          const downwardReduction = Math.max(
            0,
            originalDownwardExtent - newDownwardExtent,
          );

          // 兄弟の位置を更新する
          const allNodesToUpdate: {
            nodeId: string;
            newX: number;
            newY: number;
          }[] = [];
          ...
          setTimeout(() => {
            if (upwardReduction > 0 || downwardReduction > 0) {
              ...
              const collectContractionAdjustments = (
                currentParentId: string,
                upReduction: number,
                downReduction: number,
              ) => {
                ...
                const currentParentIndex = parentSiblings.findIndex(
                  (n) => n.id === currentParentId,
                );
                ...
                if (downReduction > 0) {
                  for (
                    let i = currentParentIndex + 1;
                    i < parentSiblings.length;
                    i++
                  ) {
                    nodesToMove.push({
                      nodeId: parentSiblings[i].id,
                      deltaY: -downReduction,
                    });
                  }
                }

                // さらに上の階層に伝播
                collectContractionAdjustments(
              ...
              if (nodesToMove.length > 0) {
                ...
                setNodes((prev) =>
                  prev.map((node) => {
                    const moveData = allNodesToMove.find(
                      (m) => m.nodeId === node.id,
                    );
                    if (moveData) {
